...
?>
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
            <i class="glyphicon glyphicon-stats"></i> &nbsp;&nbsp;Diagram Kegiatan
          </h1>
        </section>
        <?php } ?>
        <!-- Main content -->
                                    ...
                                    <div class="pull-left">
                                        <ol class="breadcrumb" style="margin-bottom: 0;">
                                            <?php if($this->uri->segment(3) != ""){ ?>
                                                ...
                                                <li><a href="<?php echo site_url();?>kegiatan/diagram_kegiatan">Diagram Kegiatan</a></li>
                                                <li class="active"><?php echo $this->uri->segment(3); ?></li>
                                            <?php }else{ ?>
                                                <li><a href="<?php echo site_url();?>"><i class="glyphicon glyphicon-home"></i> Beranda</a></li>
                                                <li class="active">Diagram Kegiatan</li>
                                            <?php }?>
                                        </ol>
                                    </div>
                                    ...
                                    <div class="box box-solid box-primary">
                                        ...
                                        <div class="box-body">
                                            <div class="btn-group">
                                                ...
                                                <?php 
                                                    $query      = $this->model_diagram->view_tbl_dokumen();
                                                    foreach($query->result() as $row): 
                                                ?>
                                                    <li>
                                                        <a href="<?php echo base_url(); ?>kegiatan/diagram_kegiatan/<?php echo substr($row->year, 0, 4); ?>">
    ...
    <script type="text/javascript">
        FusionCharts.ready(function(){
            var chart = new FusionCharts({
                type:'column2d', // column2d, column3d, bar2d, bar3d, pie2d, pie3d, doughnut2d, doughnut3d, line, area2d
                ...
                dataSource : {
                        'chart' : {
                        <?php if($this->uri->segment(3) != ""){?>
                        'caption' : 'Perolehan Jumlah Dokumen Berdasarkan Kegiatan pada Tahun <?php echo $this->uri->segment(3);?>',
                        <?php }else{ ?>
                        'caption' : 'Perolehan Jumlah Dokumen Berdasarkan Kegiatan',
                        <?php }?>
                        'xAxisName'	: 'Jenis Kegiatan',
                        'yAxisName' : 'Jumlah Dokumen',
                        'theme' : 'ocean'
                    },
                    'data' : [
                        ...
                        <?php
                            if($this->uri->segment(3) != ""){
                                $query    = $this->model_diagram->view_diagram_kegiatan_filter($this->uri->segment(3));
                                $i        = 0;
                                foreach($query->result() as $row) :
                                    if($i++)
                                    echo ",";
                        ?>
                            {'label':'<?php echo $row->jenis_kegiatan;?>', 'value':'<?php echo $row->jumlah;?>'}
                        <?php
                                endforeach;
                            }else{
                                $query    = $this->model_diagram->view_diagram_kegiatan();
                                $i        = 0;
                                foreach($query->result() as $row) :
                                    if($i++)
                                    echo ",";
